...
/**
 * {@code ListenerBase} is the programming language independent interface of all listeners.
 *
...
public interface ListenerBase extends ParseTreeListener {
    ...
    default String calculateChecksum(String sourceCode) {
        try {
            byte[] checksum = MessageDigest.getInstance("MD5").digest(sourceCode.getBytes(UTF_8));
